...
var recurringJob = schedule.scheduleJob(rule, function() {
    request(url, function(error, response, body) {
        if (!error) {
            //GRAB THE LATEST WINNING NUMBERS
            ...
            var theDrawDate = new Date(dateTrimmed[0])
                .toISOString()
                .split('T')[0];
            ...
            function callback(error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    for (var i = 0; i < info.length; i++) {
                        apiDate = new Date(info[i].drawDate)
                            .toISOString()
                            .split('T')[0];
                        ...
                        function callbackPost(error, response, body) {
                            if (!error && response.statusCode == 200) {
                                ...
                                var optionsMyNumbersGet = {
                                    url:
                                        sUrl +
                                        '/api/lottery/v1/lottery/mynumbers',
                                    method: 'GET',
                                    json: true
                                };

                                function callbackMyNumbersGet(
                                    error,
                                    response,
                                    body
                                ) {
                                    if (!error && response.statusCode == 200) {
                                        ...
                                        for (
                                            var i = 0;
                                            i < infoMyNumbers.length;
                                            i++
                                        ) {
                                            matchedNumbers = [];
                                            myNumbersId = infoMyNumbers[i]._id;
                                            myWinners = '';
                                            apiMyNumbersDate = new Date(
                                                infoMyNumbers[i].drawDate
                                            )
                                                .toISOString()
                                                .split('T')[0];

                                            if (
                                                apiMyNumbersDate == theDrawDate
                                            ) {
                                                ...
                                                matchedNumbers = arrayCompare.intersect(
                                                    MyNumbers,
                                                    winningNumsArray
                                                );
